...
const createMockResponse = (ok: boolean, data: any, status = 200, statusText = 'OK') => {
  return {
    ok,
    status,
    statusText,
...
describe('Informatica Mapping Service', () => {
  ...
  describe('authenticateWithInformatica', () => {
    it('should authenticate successfully', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createMockResponse(
        true, 
        { sessionId: 'mock-session-id', orgId: 'mock-org-id' }
      ));
  ...
  describe('importMappingDocumentation', () => {
    ...
    it('should handle import failure', async () => {
      ...
      expect(result).toEqual({
        success: false,
        error: 'Failed to import mapping documentation: {"error":"Invalid data format"}'
      });
    });
  });
  ...
  describe('checkImportJobStatus', () => {
    ...
    it('should check job status successfully - FAILED', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createMockResponse(
        true,
        { status: 'FAILED', error: 'Import job failed' }
      ));
