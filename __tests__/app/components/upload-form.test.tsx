...
jest.mock('@/app/components/auth-form', () => ({
  ...
  default: jest.fn(({ onSubmit }) => (
    <div data-testid="mock-auth-form">
      <button onClick={() => onSubmit({ username: 'test', password: 'test', baseUrl: 'url', apiUrl: 'api' })}>
        Submit Auth
      </button>
    </div>
  ))
...
jest.mock('@/app/components/file-uploader', () => ({
  ...
  default: jest.fn(({ onFileChange, onSubmit }) => (
    <div data-testid="mock-file-uploader">
      <button onClick={() => onFileChange(new File(['test'], 'test.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }))}>
        Change File
      </button>
      <button onClick={() => onSubmit({ file: [new File(['test'], 'test.xlsx')], description: 'test' })}>
        Submit File
      </button>
    </div>
  ))
