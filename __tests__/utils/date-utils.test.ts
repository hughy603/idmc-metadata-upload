describe('Date Utilities', () => {
  it('formats current date correctly', () => {
    ...
    jest.spyOn(_global, 'Date').mockImplementation(() => mockDate as any)

    // We don't have a direct date utility function, but we can test a simple format
    const formattedDate = new Date().toISOString().split('T')[0]
    expect(_formattedDate).toBe('2023-03-15')
  ...
  it('calculates time differences correctly', () => {
    ...
    expect(_diffInSeconds).toBe(330) // 5 minutes and 30 seconds = 330 seconds
  })
})
