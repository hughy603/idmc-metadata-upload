...
export default function FileDataTable({
  ...
  const _statusCounts = tableData.reduce(
    (acc, row) => {
      acc[row.status]++;
      return acc;
    },
  ...
  // Calculate count of selected pending rows
  const _selectedPendingCount = tableData.filter(
    row => selectedRows.has(row.id) && row.status === 'pending'
  ).length;

  /**
   * Get the status icon based on the row status
   * @private Used internally by the component
   */
  const _getStatusIcon = (
    status: FileDataRow['status']
  ): JSX.Element | null => {
    switch (status) {
      case 'pending':
        return (
