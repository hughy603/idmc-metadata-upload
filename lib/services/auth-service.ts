...
import type { InformaticaAuthCredentials, InformaticaSession, InformaticaToken } from '@/lib/utils/auth';
import { generateJwtToken, getAuthBaseUrl, getInformaticaSession, INFORMATICA_REGIONS, isTokenExpired } from '@/lib/utils/auth';
...
export class AuthService {
   ...
   */
  public async login(credentials: InformaticaAuthCredentials): Promise<boolean> {
    try {
  ...
  public initiateOAuth(redirectUri: string): string {
    ...
    const clientId = process.env.NEXT_PUBLIC_INFORMATICA_CLIENT_ID || 'default-client-id'
   ...
   */
  public async completeOAuth(code: string, redirectUri: string): Promise<boolean> {
    try {
      const baseUrl = getAuthBaseUrl(this.region)
      ...
      const clientId = process.env.NEXT_PUBLIC_INFORMATICA_CLIENT_ID || 'default-client-id'
      const clientSecret = process.env.INFORMATICA_CLIENT_SECRET || 'default-client-secret'
...
export const login = (credentials: InformaticaAuthCredentials): Promise<boolean> => authService.login(credentials)
export const logout = (): void => authService.logout()
export const getAccessToken = (): Promise<string | null> => authService.getAccessToken()
export const getAuthStatus = (): { isAuthenticated: boolean; region: string } => authService.getAuthStatus()
export const setRegion = (region: keyof typeof INFORMATICA_REGIONS): void => authService.setRegion(region)
